...
-- =============================================
CREATE PROCEDURE [dbo].[PK_dVehicleDailyReports_R]
(
	-- Parameter
	...
	BEGIN
		...
		FROM																																			
		  (SELECT TOP 1001
				...
				ISNULL(JT_Unkobi.UkeNo, 0) AS UkeNo ,																																			
				ISNULL(JT_Unkobi.UnkRen, 0) AS UnkRen ,																																			
				ISNULL(JT_Unkobi.DanTaNm, '') AS DanTaNm ,																																			
				ISNULL(JT_Haisha.DanTaNm2, '') AS DanTaNm2 ,																																			
				ISNULL(JT_Haisha.IkNm, '') AS IkNm ,																																			
			    ...
			    ISNULL(JT_Haisha.SyukoTime, '') AS Haisha_SyukoTime ,															
                TKD_Shabni.SyukoTime AS Shabni_SyukoTime ,															
                ISNULL(JT_Haisha.KikYmd, '') AS KikYmd ,															
                ISNULL(JT_Haisha.KikTime, '') AS Haisha_KikTime ,															
                TKD_Shabni.KikTime AS Shabni_KikTime ,																																													
				TKD_Shabni.StMeter AS StMeter ,																																			
				TKD_Shabni.EndMeter AS EndMeter ,																																			
				TKD_Shabni.JisaIPKm AS JisaIPKm ,																																			
				TKD_Shabni.JisaKSKm AS JisaKSKm ,																																			
				TKD_Shabni.KisoIPKm AS KisoIPKm ,																																			
				TKD_Shabni.KisoKOKm AS KisoKSKm ,																																			
				TKD_Shabni.OthKm AS OthKm ,																																			
				...
				TKD_Shabni.Nenryo3 AS Nenryo3 ,																																			
				TKD_Shabni.JyoSyaJin ,																																			
				TKD_Shabni.PlusJin ,																																			
				TKD_Shabni.UnkKai ,																																	
		   ...
		   FROM TKD_Shabni																																			
				...
				LEFT JOIN VP_Haisha1 AS JW_Haisha ON TKD_Shabni.UkeNo = JW_Haisha.UkeNo																	
