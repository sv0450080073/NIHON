...
/****** Object:  StoredProcedure [dbo].[Pro_GetInfoHaiinDrive_R]    Script Date: 8/7/2020 12:53:39 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[Pro_GetInfoHaiinDrive_R]
    (	
		@Ukeno VARCHAR(20)
	,	@UnkRen VARCHAR(10)
	,	@TeiDanNo VARCHAR(10)
...
BEGIN TRY
		...
		' JOIN VPM_KyoSHe KYOSHE_DR ON HAIIN_DR.SyainCdSeq = KYOSHE_DR.SyainCdSeq 
			   AND KYOSHE_DR.SyokumuCdSeq IN(1, 2) '	+
		...
		' JOIN VPM_KyoSHe KYOSHE_GUI ON HAIIN_GUI.SyainCdSeq = KYOSHE_GUI.SyainCdSeq 
			   AND KYOSHE_GUI.SyokumuCdSeq IN(3,4) '	+
		...
		IF OBJECT_ID('tempdb.dbo.##tb_HaiinDRTemp', 'U') IS  NULL
			BEGIN
			CREATE TABLE ##tb_HaiinDRTemp (rowIndex int IDENTITY(1,1)
								,UkeNo VARCHAR(15)
								, SyainCdSeq INT
								, TeiDanNo SMALLINT
								, SyokumuCdSeq INT
		...
		IF OBJECT_ID('tempdb.dbo.##tb_HaiinGUITemp', 'U') IS  NULL
			BEGIN
			CREATE TABLE ##tb_HaiinGUITemp (rowIndex int IDENTITY(1,1)
								,UkeNo VARCHAR(15)
								, SyainCdSeq INT
								, TeiDanNo SMALLINT
								, SyokumuCdSeq INT
		...
		SET @strSQL=''
		SET @strSQL = @strSQL + 
		'SELECT ISNULL(HAIIN_DR.SyainCd,0) AS DR_SyainCd, ISNULL(HAIIN_DR.SyainNm,'''') AS DR_SyainNm,
				ISNULL(HAIIN_GUI.SyainCd,0) AS GUI_SyainCd,ISNULL(HAIIN_GUI.SyainNm,'''') AS GUI_SyainNm, 
				ISNULL(HAIIN_DR.TeiDanNo,0) AS TeiDanNo   
		FROM ##tb_HaiinDRTemp HAIIN_DR LEFT JOIN ##tb_HaiinGUITemp HAIIN_GUI    
			ON HAIIN_DR.rowIndex=HAIIN_GUI.rowIndex'

		Declare @tb_HAIIN TABLE (DR_SyainCd CHAR(255),DR_SyainNm NVARCHAR(255),GUI_SyainCd CHAR(255), GUI_SyainNm NVARCHAR(255),TeiDanNo SMALLINT)
