USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[PK_dBillingListGridWithoutOutputClassification_R]    Script Date: 4/28/2021 10:26:01 AM ******/
SET ANSI_NULLS ON
GO
...
------------------------------------------------------------
CREATE OR ALTER PROCEDURE [dbo].[PK_dBillingListGridWithoutOutputClassification_R]
		(
		--Parameter
			@TenantCdSeq			INT,
			@BillDate				NVARCHAR(6),
...
AS
	-- Processing
BEGIN
	IF OBJECT_ID(N'tempdb..#BillingListGrid') IS NOT NULL
	BEGIN
	DROP TABLE #BillingListGrid
	END
-- 入金支払明細テーブル
...
SELECT DISTINCT
--DISTINCT COUNT(*) OVER(ORDER BY (SELECT NULL)) AS CountNumber,
-- 請求営業所
...
SELECT ISNULL(COUNT(*), 0) as CountNumber FROM #BillingListGrid

SELECT SUM(#BillingListGrid.UriUriGakKin) as UriUriGakKinSum,
SUM(#BillingListGrid.UriSyaRyoSyo) as UriSyaRyoSyoSum,
SUM(#BillingListGrid.UriSyaRyoTes) as UriSyaRyoTesSum,
SUM(#BillingListGrid.UriSeiKin) as UriSeiKinSum,
SUM(#BillingListGrid.UriNyuKinRui) as UriNyuKinRuiSum,
SUM(#BillingListGrid.UriSeiKin - #BillingListGrid.UriNyuKinRui - #BillingListGrid.UriMaeuke) as UriSum,
SUM(#BillingListGrid.UriMaeuke) as UriMaeukeSum,
SUM(#BillingListGrid.GaiGaiGakKin) as GaiGaiGakKinSum,
SUM(#BillingListGrid.GaiSyaRyoSyo) as GaiSyaRyoSyoSum,
SUM(#BillingListGrid.GaiSyaRyoTes) as GaiSyaRyoTesSum,
SUM(#BillingListGrid.GaiSeiKin) as GaiSeiKinSum,
SUM(#BillingListGrid.GaiNyuKinRui) as GaiNyuKinRuiSum,
SUM(#BillingListGrid.GaiSeiKin - #BillingListGrid.GaiNyuKinRui - #BillingListGrid.GaiMaeuke) as GaiSum,
SUM(#BillingListGrid.GaiMaeuke) as GaiMaeukeSum,
SUM(#BillingListGrid.EtcEtcGakKin) as EtcEtcGakKinSum,
SUM(#BillingListGrid.EtcSyaRyoSyo) as EtcSyaRyoSyoSum,
SUM(#BillingListGrid.EtcSyaRyoTes) as EtcSyaRyoTesSum,
SUM(#BillingListGrid.EtcSeiKin) as EtcSeiKinSum,
SUM(#BillingListGrid.EtcNyuKinRui) as EtcNyuKinRuiSum,
SUM(#BillingListGrid.EtcSeiKin - #BillingListGrid.EtcNyuKinRui - #BillingListGrid.EtcMaeuke) as EtcSum,
SUM(#BillingListGrid.EtcMaeuke) as EtcMaeukeSum,
SUM(#BillingListGrid.CanCanGakKin) as CanCanGakKinSum,
SUM(#BillingListGrid.CanSyaRyoSyo) as CanSyaRyoSyoSum,
SUM(#BillingListGrid.CanSeiKin) as CanSeiKinSum,
SUM(#BillingListGrid.CanNyuKinRui) as CanNyuKinRuiSum,
SUM(#BillingListGrid.CanSeiKin - #BillingListGrid.CanNyuKinRui - #BillingListGrid.CanMaeuke) as CanSum,
SUM(#BillingListGrid.CanMaeuke) as CanMaeukeSum
FROM #BillingListGrid
