USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[Pro_VenderRequestMain_R]    Script Date: 6/7/2021 11:18:38 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dbo].[Pro_VenderRequestMain_R]
		...
		@customerStart varchar(11),
		@customerEnd varchar(11),
		@tenantId int
AS
BEGIN
...
SELECT 
	  ...
	  ,YOU_TOKISK.TokuiCd AS TokuiCd
	  ,YOU_TOKIST.SitenCd AS SitenCd
	  ,YOU_GYOSYA.GyosyaCd AS GyosyaCd
	  ,YOU_GYOSYA.GyosyaNm AS GyosyaNm
FROM 
...
WHERE  
    ...
    AND (FORMAT(YOU_GYOSYA.GyosyaCd,'000') + FORMAT(YOU_TOKISK.TokuiCd,'0000') + FORMAT(YOU_TOKIST.SitenCd,'0000')) >= @customerStart 
	AND (FORMAT(YOU_GYOSYA.GyosyaCd,'000') + FORMAT(YOU_TOKISK.TokuiCd,'0000') + FORMAT(YOU_TOKIST.SitenCd,'0000')) <= @customerEnd
