USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[Pro_BusCoordination_R]    Script Date: 5/18/2021 2:16:02 PM ******/
SET ANSI_NULLS ON
GO
...
--  Update      :
--  Comment     :
--  2021/05/18 #8656 fixbug change TenantCdSeq from varchar(1) into varchar(5)
----------------------------------------------------
...
AS
	BEGIN TRY 	
      ...
      /* SET @ReturnCd   =   0       -- リターンコード初期化
		...
		if(@UkenoList!='')
		begin
		select @CheckUkeList =' And concat(UNKOBI.UkeNo,FORMAT(UNKOBI.UnkRen, ''000'')) IN (select * from FN_SplitString('+dbo.FP_SetString(@UkenoList)+', '','')) '
		end
		else
		begin
		/*CHECK SELECT ALL*/
	...
	LEFT JOIN VPM_CodeKb AS ZEIKBN																																	
       ON CAST(ZEIKBN.CodeKbn AS INT) = CAST(YYKSHO.ZeiKbn AS INT)																																
	  AND ZEIKBN.CodeSyu = ''ZEIKBN''																																
	  AND ZEIKBN.SiyoKbn = 1																																
	  AND ZEIKBN.TenantCdSeq ='+dbo.CheckCodeKbFunct('ZEIKBN',@TenantCdSeq)+'     																																
	LEFT JOIN VPM_CodeKb AS UKEJOKEN																																	
       ON CAST(UKEJOKEN.CodeKbn AS INT) = CAST(UNKOBI.UkeJyKbnCd AS INT)																																	
	  AND UKEJOKEN.TenantCdSeq ='+dbo.CheckCodeKbFunct('UKEJYKBNCD',@TenantCdSeq)+'  																															
	  AND UKEJOKEN.CodeSyu = ''UKEJYKBNCD''																															
	LEFT JOIN VPM_CodeKb AS OTHERJIN1																																	
		ON CAST(OTHERJIN1.CodeKbn AS INT) = CAST(UNKOBI.OthJinKbn1 AS INT)																																	
		AND OTHERJIN1.CodeSyu = ''OTHJINKBN''																																
		AND OTHERJIN1.TenantCdSeq ='+dbo.CheckCodeKbFunct('OTHJINKBN',@TenantCdSeq)+'        																																
	LEFT JOIN VPM_CodeKb AS OTHERJIN2																																	
		 ON CAST(OTHERJIN2.CodeKbn AS INT) = CAST(UNKOBI.OthJinKbn2 AS INT)																																	
		AND OTHERJIN2.CodeSyu = ''OTHJINKBN''																																
		 AND OTHERJIN2.TenantCdSeq ='+dbo.CheckCodeKbFunct('OTHJINKBN',@TenantCdSeq)+' 
	LEFT JOIN VPM_CodeKb AS SJJOKBN1																							
		ON CAST(SJJOKBN1.CodeKbn AS INT) = CAST(UNKOBI.SijJoKbn1 AS INT)																							
		AND SJJOKBN1.TenantCdSeq ='+dbo.CheckCodeKbFunct('SIJJOKBN1',@TenantCdSeq)+'                																						
		AND SJJOKBN1.CodeSyu = ''SIJJOKBN1''																						
	LEFT JOIN VPM_CodeKb AS SJJOKBN2																							
		ON CAST(SJJOKBN2.CodeKbn AS INT) = CAST(UNKOBI.SijJoKbn2 AS INT)																							
		AND SJJOKBN2.TenantCdSeq ='+dbo.CheckCodeKbFunct('SIJJOKBN2',@TenantCdSeq)+'             																					
		AND SJJOKBN2.CodeSyu = ''SIJJOKBN2''																					
	LEFT JOIN VPM_CodeKb AS SJJOKBN3																							
		ON CAST(SJJOKBN3.CodeKbn AS INT) = CAST(UNKOBI.SijJoKbn3 AS INT)																							
		AND SJJOKBN3.TenantCdSeq ='+dbo.CheckCodeKbFunct('SIJJOKBN3',@TenantCdSeq)+'              																					
		AND SJJOKBN3.CodeSyu = ''SIJJOKBN3''																						
	LEFT JOIN VPM_CodeKb AS SJJOKBN4																							
		ON CAST(SJJOKBN4.CodeKbn AS INT) = CAST(UNKOBI.SijJoKbn4 AS INT)																							
		AND SJJOKBN4.TenantCdSeq ='+dbo.CheckCodeKbFunct('SIJJOKBN4',@TenantCdSeq)+'               																						
		AND SJJOKBN4.CodeSyu = ''SIJJOKBN4''																					
	LEFT JOIN VPM_CodeKb AS SJJOKBN5																							
		ON CAST(SJJOKBN5.CodeKbn AS INT) = CAST(UNKOBI.SijJoKbn5 AS INT)																							
		AND SJJOKBN5.TenantCdSeq ='+dbo.CheckCodeKbFunct('SIJJOKBN5',@TenantCdSeq)+'                																					
