...
/****** Object:  StoredProcedure [dbo].[Pro_GetInfoBooking_R]    Script Date: 5/21/2021 10:59:56 AM ******/
SET ANSI_NULLS ON
GO
...
AS
	...
	' SELECT	ISNULL(HAISHA.UkeNo,'''') AS UkeNo '	+
			...
			' ,	CONVERT(INT,CONVERT(DATETIME, '+dbo.FP_SetString(@DateBooking)+')-CONVERT(DATETIME, HAISHA.SyuKoYmd))+1 AS DayBusRunning '	+
			' ,	CONVERT(INT,CONVERT(DATETIME, HAISHA.KikYmd)-CONVERT(DATETIME, HAISHA.SyuKoYmd))+1 AS TotalDayBusRun '	+
			' ,'''' AS VistLocation '	+
			' ,'''' AS VistLocationCompact '	+
			...
			' ,	(SELECT SUM(SyaSyuDai) FROM TKD_YykSyu WHERE UkeNo =HAISHA.UkeNo AND SiyoKbn=1 GROUP BY TKD_YykSyu.UkeNo) AS TotalBus '	+
			' ,	CONVERT(INT,CONVERT(DATETIME, HAISHA.KikYmd)-CONVERT(DATETIME, HAISHA.SyuKoYmd))+1 AS TotalDayBusRunReal '	+
			' ,	CONVERT(INT,CONVERT(DATETIME, '+dbo.FP_SetString(@DateBooking)+')-CONVERT(DATETIME, HAISHA.SyuKoYmd))+1 AS DayBusRunningReal '	+								
			' , ' + @RowNumberCondition +''+
		...
		' LEFT JOIN VPM_HenSya AS HENSYA ON HENSYA.SyaRyoCdSeq = HAISHA.HaiSSryCdSeq  
			AND HENSYA.StaYmd <=UNKOBI.HaiSYmd
			AND HENSYA.EndYmd >=UNKOBI.HaiSYmd  '	+
		...
		' LEFT JOIN VPM_SyaSyu AS SYASYU ON SYASYU.SyaSyuCdSeq = SYARYO.SyaSyuCdSeq AND SYASYU.TenantCdSeq =ISNULL( '+@TenantCdSeq+','+@TenantCdSeq+') '	+
		' WHERE HAISHA.SiyoKbn = 1 '	+
		' AND YOYAKUSHO.YoyaSyu = 1 '+
		'		AND HAISHA.HaiSSryCdSeq <> 0 '	+
		'		AND YOYAKUSHO.TenantCdSeq =ISNULL( '+@TenantCdSeq+','+@TenantCdSeq+') '	+
		'		AND COMPANY.TenantCdSeq =ISNULL( '+@TenantCdSeq+','+@TenantCdSeq+') '	+
		...
		DECLARE @tb_Temp TABLE(   UkeNo VARCHAR(15)
										...
										, TotalBus VARCHAR(10)
										, TotalDayBusRunReal INT
										, DayBusRunningReal INT
										, RowNum VARCHAR(10)										
								      ) 	
