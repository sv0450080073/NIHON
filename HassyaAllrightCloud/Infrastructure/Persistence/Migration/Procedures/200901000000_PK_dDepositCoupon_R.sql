...
AS
	...
	DECLARE @wk_Offset		NVARCHAR(10)
	DECLARE @wk_Limit		NVARCHAR(10)
	...
	SET @wk_Offset	= CONVERT(NVARCHAR(10) ,@Offset)               
	SET @wk_Limit	= CONVERT(NVARCHAR(10) ,@Limit)               
	SET @strSQL = ' '
	SET	@RowCount =	0
	-- Processing
BEGIN
...
WHERE eTKD_Yyksho01.TenantCdSeq = ' + @wk_TenantCdSeq -- ログインしたユーザーのTenantCdSeq
	...
	IF(TRIM(@wk_GyosyaCd) <> '0' AND TRIM(@wk_TokuiCd) <> '0' AND TRIM(@wk_SitenCd) <> '0' AND TRIM(@wk_TokuiNm) <> '')
	BEGIN
	...
	SET @strSQL = @strSQL + ' ORDER BY eTKD_Yyksho01.SeiTaiYmd ASC,
	eTKD_Yyksho01.UkeNo ASC,
	eTKD_Mishum02.SeiFutSyu ASC,
	eTKD_Mishum02.FutTumRen ASC
	OFFSET ' + @wk_Offset + ' ROWS '
	IF(@wk_Limit <> '-1')
	BEGIN
	SET @strSQL = @strSQL + ' FETCH NEXT ' + @wk_Limit + ' ROWS ONLY '
	END
EXEC	(@strSQL)
