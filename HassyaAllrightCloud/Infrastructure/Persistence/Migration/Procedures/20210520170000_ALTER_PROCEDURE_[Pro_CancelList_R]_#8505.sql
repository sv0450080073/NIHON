...
/****** Object:  StoredProcedure [dbo].[Pro_CancelList_R]    Script Date: 2021/05/28 14:17:16 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[Pro_CancelList_R] 
...
SELECT  Yo.UkeCd																												
       ,Un.UnkRen																											
       ,Yo.YoyaKbnSeq
	   ,YoyaKbn.YoyaKbn AS YoyaCodeTokuiTanStaff
       ,YoyaKbn.YoyaKbnNm																												
       ,Yo.UkeYmd																											
       ,Toku.RyakuNm AS TokuRyakuNm																										
       ,Toku.TokuiNm TokuTokuiNm
	   ...
	   ,Yo.CanZKbn AS CanZKbn
	   ,Yo.CanSyoR AS CanSyoR
	   ,'CanZRyakuNm' as CanZRyakuNm
...
　　　  ,Un.TouNm AS TouNm																												
	   ...
	   ,'CodeKb.RyakuNm' as CodeKbRyakuNm
...
INNER JOIN VPM_Tokisk AS Toku																												
       ON Yo.TokuiSeq=Toku.TokuiSeq and Toku.TenantCdSeq = @tenantId
INNER JOIN VPM_TokiSt AS Toshi　																												
      ON Yo.SitenCdSeq=Toshi.SitenCdSeq And Yo.TokuiSeq = Toshi.TokuiSeq																												
...
LEFT JOIN VPM_Gyosya AS Gyosa																												
       ON Gyosa.GyosyaCdSeq=Toku.GyosyaCdSeq and Gyosa.TenantCdSeq = @tenantId
LEFT JOIN VPM_Gyosya AS Gosa																												
       ON Gosa.GyosyaCdSeq=Toki.GyosyaCdSeq and Gosa.TenantCdSeq = @tenantId
...
LEFT JOIN VPM_SyaSyu AS Sashu																												
       ON Shu.SyaSyuCdSeq=Sashu.SyaSyuCdSeq and Sashu.TenantCdSeq = @tenantId																										
LEFT JOIN VPM_YoyKbn AS YoyaKbn																												
       ON YoyaKbn.YoyaKbnSeq = Yo.YoyaKbnSeq and YoyaKbn.TenantCdSeq = @tenantId
