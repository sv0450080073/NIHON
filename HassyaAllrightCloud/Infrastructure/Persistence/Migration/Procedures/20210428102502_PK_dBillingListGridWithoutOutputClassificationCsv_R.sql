USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[PK_dBillingListGridWithoutOutputClassificationCsv_R]    Script Date: 4/28/2021 10:27:20 AM ******/
SET ANSI_NULLS ON
GO
...
------------------------------------------------------------
CREATE OR ALTER PROCEDURE [dbo].[PK_dBillingListGridWithoutOutputClassificationCsv_R]
		(
		--Parameter
			@TenantCdSeq			INT,
			@BillDate				NVARCHAR(6),
...
-- キャンセル料・前受金集計
	...
	ISNULL(eTKD_Kuri11.UriZenZan, 0) + ISNULL(eTKD_Kuri11.GaiZenZan, 0) + ISNULL(eTKD_Kuri11.EtcZenZan, 0) + ISNULL(eTKD_Kuri11.CanZenZan, 0) AS UGECZenZan,
		SUM(
		CASE
			WHEN TKD_Mishum.SeiFutSyu IN (1) THEN CAST(TKD_Mishum.UriGakKin as bigint)
			ELSE 0
		...
		CASE
            ...
            ELSE 0
        END
	) + MAX(ISNULL(eTKD_NyShmi12.EtcNyukinG, 0)) +     MAX(
		CASE
            WHEN TKD_Mishum.SeiFutSyu IN(7) THEN ISNULL(eTKD_NyShmi11.NyukinG, 0)
            ELSE 0
        END
	...
	--
	ISNULL(eTKD_Kuri11.UriZenZan, 0) + ISNULL(eTKD_Kuri11.GaiZenZan, 0) + ISNULL(eTKD_Kuri11.CanZenZan, 0) AS UGCZenZan,
		SUM(
		CASE
			WHEN TKD_Mishum.SeiFutSyu IN (1) THEN CAST(TKD_Mishum.UriGakKin as bigint)
			ELSE 0
