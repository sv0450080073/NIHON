...
/****** Object:  StoredProcedure [dbo].[Pro_GetInfoBooking_R]    Script Date: 2/26/2021 9:55:25 AM ******/
SET ANSI_NULLS ON
GO
...
AS
	BEGIN TRY
		...
		SELECT @BranchCondition =	CASE
										...
										ELSE 'AND EIGYOSHO.EigyoCd >='+@BranchFrom+'
											  AND EIGYOSHO.EigyoCd <='+@BranchTo+''
									END
	...
	' SELECT	ISNULL(HAISHA.UkeNo,'''') AS UkeNo '	+
		...
		' LEFT JOIN VPM_YoyKbn AS YOYKBN ON YOYKBN.YoyaKbnSeq =  YOYAKUSHO.YoyaKbnSeq AND YOYKBN.SiyoKbn = 1 '	+
