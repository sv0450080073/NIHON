USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[PK_dAdvancePaymentDetailChildList_R]    Script Date: 4/22/2021 9:17:12 AM ******/
SET ANSI_NULLS ON
GO
...
-- SP-ID		:	[PK_dAdvancePaymentDetailChildList_R]
-- DB-Name		:	[HOC_Kashikiri]
-- Name			:	立替明細書内容データ取得
-- Date			:	2020/09/21
-- Author		:	nhhkieuanh, ntrunghieu update
-- Description	:	立替明細書内容データ取得
---------------------------------------------------
-- Update		: NTLanAnh-2020/12/14
-- Comment		: Update where conditions clause
----------------------------------------------------

CREATE OR ALTER PROCEDURE [dbo].[PK_dAdvancePaymentDetailChildList_R]
	(
...
BEGIN
	SELECT ISNULL(eTKD_Yyksho11.UkeNo, 0) AS UkeNo,
		...
		ISNULL(eTKD_FutTum11.FutTumKbn, 0) AS FutTumKbn,
		ISNULL(eTKD_FutTum11.FutTumRen, 0) AS FutTumRen,
		ISNULL(eTKD_Unkobi11.HaiSYmd, '') AS HaiSYmd,
		ISNULL(eTKD_Unkobi11.TouYmd, '') AS TouYmd,
		CASE
			WHEN eTKD_MFutTu11.Suryo <> 0 THEN ISNULL(eTKD_Haisha11.GoSya, '')
			ELSE ''
		END AS GoSya,
		ISNULL(eTKD_FutTum11.SeisanKbn, 0) AS SeisanKbn,
		ISNULL(eVPM_Seisan11.SeisanCd, 0) AS SeisanCd,
		ISNULL(eTKD_FutTum11.SeisanNm, '') AS SeisanNm,
		ISNULL(eTKD_FutTum11.HasYmd, '') AS UnkYmd,
		ISNULL(eTKD_FutTum11.BikoNm, '') AS RiyoDouro,
		ISNULL(eTKD_FutTum11.IriRyoNm, 0) AS IriRyoNm,
		ISNULL(eTKD_FutTum11.DeRyoNm, 0) AS DeRyoNm,
		ISNULL(eTKD_FutTum11.TanKa, 0) AS Tanka,
		ISNULL(eTKD_MFutTu11.Suryo, 0) AS Suryo,
		CASE
			WHEN eTKD_MFutTu11.Suryo <> 0 THEN ISNULL(eTKD_MFutTu11.UriGakKin, 0) + ISNULL(eTKD_MFutTu11.SyaRyoSyo, 0)
			ELSE ISNULL(eTKD_FutTum11.UriGakKin, 0) + ISNULL(eTKD_FutTum11.SyaRyoSyo, 0)
		END AS Kingaku,
		ISNULL(eTKD_FutTum11.ExpItem, 0) AS ExpItem,
		CASE
			WHEN eVPM_Futai11.FutaiCd IN (1, 2) THEN 1
			ELSE ISNULL(eVPM_Futai11.FutaiCd, 0)
		END AS FutaiCd,
		ISNULL(eTKD_FutTum11.FutTumNm, '') AS FutTumNm,
		ISNULL(eTKD_FutTum11.BikoNm, '') AS BikoNm,
		CASE
			WHEN eVPM_Futai11.FutGuiKbn = 3 THEN 0
			ELSE 1
		END AS FutGuiKbn
	...
	LEFT JOIN VPM_Seisan AS eVPM_Seisan11 
		ON eVPM_Seisan11.SeisanCdSeq = eTKD_FutTum11.SeisanCdSeq
	...
	　　　　　--AND (@EndBillAddress IS NULL OR @EndBillAddress = '' OR FORMAT(eVPM_Gyosya12.GyosyaCd, '000') + FORMAT(eVPM_Tokisk12.TokuiCd, '0000') + FORMAT(eVPM_TokiSt12.SitenCd, '0000') <= @EndBillAddress)        -- 請求先 終了
			 AND (FORMAT(eVPM_Gyosya12.GyosyaCd,'000') + FORMAT(eVPM_Tokisk12.TokuiCd,'0000') + FORMAT(eVPM_TokiSt12.SitenCd,'0000')) >= (FORMAT(@GyosyaFrom,'000') + FORMAT(@TokuiFrom,'0000') + FORMAT(@SitenFrom,'0000')) 																																				
	         AND (FORMAT(eVPM_Gyosya12.GyosyaCd,'000') + FORMAT(eVPM_Tokisk12.TokuiCd,'0000') + FORMAT(eVPM_TokiSt12.SitenCd,'0000')) <= (FORMAT(@GyosyaTo,'000') + FORMAT(@TokuiTo,'0000') + FORMAT(@SitenTo,'0000'))	
