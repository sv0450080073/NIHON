USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[RP_UnkoushijishoReportNew]    Script Date: 2021/06/11 18:38:24 ******/
SET ANSI_NULLS ON
GO
...
-- SP-ID		:   RP_UnkoushijishoReportNew
...
PROCEDURE [dbo].[RP_UnkoushijishoReportNew]
	...
	if(@UkenoList!='')
        ...
        WHERE
						...
						AND (@YoyakuFrom = 0 OR YoyKbn.YoyaKbnSeq >= @YoyakuFrom)
						AND (@YoyakuTo = 0 OR YoyKbn.YoyaKbnSeq <= @YoyakuTo)
                        AND HAISHA.KSKbn      <> 1               --未仮車以外            	
                        AND HAISHA.YouTblSeq   = 0               --傭車以外            	
