USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[PK_SpMnYykReport_R]    Script Date: 2021/04/07 9:28:05 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER     PROCEDURE [dbo].[PK_SpMnYykReport_R]
	@TenantCdSeq int,
    @CompanyCdSeq int,
...
BEGIN
	...
	BEGIN
        ...
        SELECT eTKD_Yyksho01.UkeNo AS UkeNo,														-- 受付番号
			...
			ISNULL(eTKD_Kaknin01.Cnt_SaikFlg	,0		)	AS		Cnt_SaikFlg,				    -- 催行フラグ集計値
	        ISNULL(eTKD_Kaknin01.Cnt_DaiSuFlg	,0		)	AS		Cnt_DaiSuFlg,			        -- 台数フラグ集計値
			ISNULL(eTKD_Kaknin01.Cnt_KingFlg	,0		)	AS		Cnt_KingFlg,				    -- 金額フラグ集計値
			ISNULL(eTKD_Kaknin01.Cnt_NitteiFlg	,0		)	AS		Cnt_NitteiFlg,			        -- 日程フラグ集計値
		...
		LEFT JOIN																				
			(																					
				SELECT 																			
						eTKD_Kaknin01.UkeNo 													
					,	SUM(eTKD_Kaknin01.SaikFlg	)	AS	Cnt_SaikFlg							
					,	SUM(eTKD_Kaknin01.DaiSuFlg	)	AS	Cnt_DaiSuFlg						
					,	SUM(eTKD_Kaknin01.KingFlg	)	AS	Cnt_KingFlg							
					,	SUM(eTKD_Kaknin01.NitteiFlg	)	AS	Cnt_NitteiFlg						
				FROM																			
						TKD_Kaknin				AS	eTKD_Kaknin01								
				WHERE																			
						eTKD_Kaknin01.SiyoKbn	=	1											
				GROUP BY																		
						eTKD_Kaknin01.UkeNo														
			)					AS	eTKD_Kaknin01	ON	eTKD_Yyksho01.UkeNo			=		eTKD_Kaknin01.UkeNo
		-- ＜取得＞(eTKD_SeiPrS01)
