...
BEGIN
	...
	BEGIN
		...
		eTKD_UnkobiFile AS (
			SELECT MAX(TKD_UnkobiFile.UpdYmd + TKD_UnkobiFile.UpdTime) AS UpdTime,
				TKD_UnkobiFile.UkeNo,
				TKD_UnkobiFile.UnkRen
			FROM TKD_UnkobiFile
			GROUP BY TKD_UnkobiFile.UkeNo,
				TKD_UnkobiFile.UnkRen
		)
        ...
        SELECT eTKD_Yyksho01.UkeNo AS UkeNo,														-- 受付番号
			...
			CAST(eTKD_Yyksho01.GuitKin AS INT) AS Gui_UriGakKin_S,									-- ガイド料_売上額集計値
			CAST(eTKD_Yyksho01.TaxGuider AS INT) AS Gui_SyaRyoSyo_S,								-- ガイド料_消費税額集計値
			CAST(eTKD_Yyksho01.FeeGuider AS INT) AS Gui_SyaRyoTes_S,								-- ガイド料_手数料額集計値
			...
			eTKD_Yyksho01.UpdYmd + eTKD_Yyksho01.UpdTime AS yykSho_UpdTime,
			ISNULL(eTKD_UnkobiFile.UpdTime, '') AS unkoFile_UpdTime
		FROM TKD_Yyksho AS eTKD_Yyksho01
		LEFT JOIN TKD_Unkobi AS eTKD_Unkobi01
			ON eTKD_Yyksho01.UkeNo = eTKD_Unkobi01.UkeNo
			AND eTKD_Unkobi01.SiyoKbn = 1
		...
		LEFT JOIN eVPM_CodeKb AS eVPM_CodeKb10
			ON eVPM_Basyo03.BasyoKenCdSeq = eVPM_CodeKb10.CodeKbnSeq
		LEFT JOIN eTKD_UnkobiFile
			ON eTKD_Unkobi01.UkeNo = eTKD_UnkobiFile.UkeNo
			AND eTKD_Unkobi01.UnkRen = eTKD_UnkobiFile.UnkRen
            /*取得条件*/
        WHERE
            eTKD_Yyksho01.YoyaSyu IN (1, 3)
            AND eTKD_Yyksho01.TenantCdSeq = @LocTenantCdSeq
