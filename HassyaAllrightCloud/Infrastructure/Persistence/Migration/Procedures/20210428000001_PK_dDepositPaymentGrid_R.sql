USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[PK_dDepositPaymentGrid_R]    Script Date: 4/28/2021 8:23:58 AM ******/
SET ANSI_NULLS ON
GO
...
FROM (
	...
	-- 入金支払テーブル
		LEFT JOIN TKD_NyuSih AS eTKD_NyuSih01
			...
			AND eTKD_NyuSih01.TenantCdSeq = @TenantCdSeq -- ログインしたユーザーのTenantCdSeq
	-- 入金支払クーポンテーブル
	...
	-- クーポンテーブル
		LEFT JOIN TKD_Coupon AS eTKD_Coupon01
			...
			AND eTKD_Coupon01.TenantCdSeq = @TenantCdSeq -- ログインしたユーザーのTenantCdSeq
	-- 営業所マスタ
	...
	FROM (
		...
		LEFT JOIN (
			...
			INNER JOIN TKD_NyuSih AS eTKD_NyuSih
				ON TKD_NyShmi.NyuSihTblSeq = eTKD_NyuSih.NyuSihTblSeq
				AND eTKD_NyuSih.SiyoKbn = 1
				AND eTKD_NyuSih.TenantCdSeq = @TenantCdSeq -- ログインしたユーザーのTenantCdSeq
