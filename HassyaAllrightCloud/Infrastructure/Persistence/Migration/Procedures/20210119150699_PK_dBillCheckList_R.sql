USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[PK_dBillCheckList_R]    Script Date: 2021/01/06 9:11:19 ******/
SET ANSI_NULLS ON
GO
...
AS 
	...
	-- 請求営業所                                                                                                                                                                  
	INNER JOIN  ( select * from VPM_Eigyos WHERE (@BillOffice IS NULL OR EigyoCdSeq = @BillOffice)) AS eVPM_Eigyos01                                                                                                                                         
	...
	LEFT JOIN (                                                                                                                                                                    
	   ...
	   WHERE eTKD_Yyksho01.TenantCdSeq = @TenantCdSeq
		...
		AND (@BillAddress IS NULL OR (FORMAT(eVPM_Gyosya02.GyosyaCd,'000') + FORMAT(eVPM_Tokisk02.TokuiCd,'0000') +	FORMAT(eVPM_TokiSt02.SitenCd,'0000')) = @BillAddress)
		...
		ORDER BY
		 (CASE WHEN (@BillTypeOrderBy IS NOT NULL) THEN SeiEigyoCd END),
		 (CASE WHEN (@BillTypeOrderBy IS NOT NULL) THEN SeiTaiYmd END),
		 (CASE WHEN (@BillTypeOrderBy IS NOT NULL) THEN SeiCd END),
		 (CASE WHEN (@BillTypeOrderBy IS NOT NULL) THEN SeiSitenCd END),
		 (CASE WHEN (@BillTypeOrderBy IS NOT NULL) THEN SeiCdSeq END),
		 (CASE WHEN (@BillTypeOrderBy IS NOT NULL) THEN UkeNo END),
		 (CASE WHEN (@BillTypeOrderBy IS NOT NULL) THEN FutuUnkRen END),
		 (CASE WHEN (@BillTypeOrderBy IS NULL) THEN SeiEigyoCd END),
		 (CASE WHEN (@BillTypeOrderBy IS NULL) THEN SeiCd END),
		 (CASE WHEN (@BillTypeOrderBy IS NULL) THEN SeiSitenCd END),
		 (CASE WHEN (@BillTypeOrderBy IS NULL) THEN SeiCdSeq END),
		 (CASE WHEN (@BillTypeOrderBy IS NULL) THEN SeiTaiYmd END),
		 (CASE WHEN (@BillTypeOrderBy IS NULL) THEN UkeNo END),
		 (CASE WHEN (@BillTypeOrderBy IS NULL) THEN FutuUnkRen END)
