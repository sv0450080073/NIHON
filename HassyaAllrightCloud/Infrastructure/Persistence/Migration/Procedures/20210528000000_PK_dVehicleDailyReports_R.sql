USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[PK_dVehicleDailyReports_R]    Script Date: 5/28/2021 8:50:47 AM ******/
SET ANSI_NULLS ON
GO
...
AS
	BEGIN
	    ...
	    ON [dbo].[#Temp] ([UkeNo],[UnkRen],[TeiDanNo],[BunkRen])
		  ...
		  (SELECT TOP 1001
		   ...
		   FROM TKD_Haisha																																			
				...
				LEFT JOIN VPM_SyaSyu AS JM_SyaSyu ON JM_SyaRyo.SyaSyuCdSeq = JM_SyaSyu.SyaSyuCdSeq
					AND JM_SyaSyu.TenantCdSeq = @TenantCdSeq
				LEFT JOIN (																	
						SELECT CodeKbn																	
								,RyakuNm																	
						FROM VPM_CodeKb																	
				...
				LEFT JOIN VPM_Gyosya AS JM_Gyosya ON JM_Tokisk.GyosyaCdSeq = JM_Gyosya.GyosyaCdSeq													  
						AND JM_Gyosya.TenantCdSeq = JM_Tokisk.TenantCdSeq				
