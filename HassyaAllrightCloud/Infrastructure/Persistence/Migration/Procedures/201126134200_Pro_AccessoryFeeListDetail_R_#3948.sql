...
/****** Object:  StoredProcedure [dbo].[Pro_AccessoryFeeListDetail_R]    Script Date: 11/26/2020 11:29:07 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROC [dbo].[Pro_AccessoryFeeListDetail_R]
					@startDate varchar(8),
					@endDate varchar(8),
					@dateType int,
...
BEGIN
	...
	WHERE Fu.FutTumKbn=1 																													
		  ...
		  AND Yo.YoyaKbnSeq IN (CASE WHEN (SELECT COUNT(*) FROM @BookingTypesTbl) = 0 THEN (SELECT Yo.YoyaKbnSeq) -- input empty => select all
									 WHEN (SELECT COUNT(*) FROM @BookingTypesTbl) > 0 THEN (SELECT TOP 1 * FROM @BookingTypesTbl bc WHERE Yo.YoyaKbnSeq = bc.BookingType) 
		  ...
		  AND M_Futai.FutGuiKbn IN 
			  (CASE WHEN (SELECT COUNT(*) FROM @FutaiTypesTbl) = 0 THEN (M_Futai.FutGuiKbn) -- input empty => select all
