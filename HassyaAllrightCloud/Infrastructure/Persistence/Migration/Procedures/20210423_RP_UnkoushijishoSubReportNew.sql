...
/****** Object:  StoredProcedure [dbo].[RP_UnkoushijishoSubReportNew]    Script Date: 2021/04/27 15:34:05 ******/
SET ANSI_NULLS ON
GO
...
>0 begin
	...
	DECLARE @tblsubreport TABLE(dateKotei date,Koutei nvarchar(102),TehNm nvarchar(102),TehTel nvarchar(30), StrDate nvarchar(6),[ID] [int] IDENTITY(1,1) NOT NULL) WHILE (@i <= @totalRecords)
	begin
	SELECT
           @date=datehaisha
    FROM
...
BEGIN
    update @tblsubreport set StrDate=case when ID=(select MIN(ID) from  @tblsubreport where dateKotei=(select dateKotei from @tblsubreport where ID=@Counter)) then StrDate else ''end where ID=@Counter
