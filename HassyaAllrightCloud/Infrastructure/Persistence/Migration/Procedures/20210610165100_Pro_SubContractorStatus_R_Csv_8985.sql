USE [HOC_Kashikiri]
GO
/****** Object:  StoredProcedure [dbo].[Pro_SubContractorStatus_R_Csv]    Script Date: 10/06/2021 9:55:12 CH ******/
SET ANSI_NULLS ON
GO
...
INNER JOIN											
  (SELECT TKD_Yousha.UkeNo ,											
   ...
   --   AND ((@tokuiFrom = 0 and @tokuiTo = 0)
     ...
     AND (FORMAT(JM_YouGyosya.GyosyaCd,'000') + FORMAT(TKD_Yousha.YouCdSeq,'0000') + FORMAT(TKD_Yousha.YouSitCdSeq,'0000')) >= (FORMAT(@gyosyaFrom,'000') + FORMAT(@tokuiFrom,'0000') + FORMAT(@sitenFrom,'0000')) 																																				
	 AND (FORMAT(JM_YouGyosya.GyosyaCd,'000') + FORMAT(TKD_Yousha.YouCdSeq,'0000') + FORMAT(TKD_Yousha.YouSitCdSeq,'0000')) <= (FORMAT(@gyosyaTo,'000') + FORMAT(@tokuiTo,'0000') + FORMAT(@sitenTo,'0000'))	
     ...
     --AND JM_YoyKbn.YoyaKbnSeq IN (SELECT * FROM FN_SplitString(@bookingTypes, '-'))	--画面で予約区分項目でのFromの番号										 
      AND @bookingTypeFrom = 0 OR JM_YoyKbn.YoyaKbn >= @bookingTypeFrom
	 AND @bookingTypeTo = 0 OR JM_YoyKbn.YoyaKbn <= @bookingTypeTo
