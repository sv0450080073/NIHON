...
/****** Object:  StoredProcedure [dbo].[Pro_StatusConfirmation_R]    Script Date: 2021/06/18 17:23:20 ******/
SET ANSI_NULLS ON
GO
...
-- Date			:   
-- Author		:   
-- Description	:   Fix duplicate syasyu, yoykbn and money yyksho 
------------------------------------------------------------
-- Editor: DC Nguyen
-- Date: 2021/06/18
------------------------------------------------------------
...
ALTER   PROCEDURE [dbo].[Pro_StatusConfirmation_R]
...
IsNull(syasyu.SyaSyuNm, 'Žw’è‚È‚µ') AS SyaSyuNm, KataKbn.RyakuNm AS KataKbn, yyksyu.SyaSyuDai AS SyaSyuDai,
...
FROM @selectedList tbl
INNER JOIN TKD_YykSyu yyksyu ON tbl.UkeNo = yyksyu.UkeNo AND tbl.UnKren = yyksyu.UnkRen
...
LEFT JOIN VPM_SyaSyu syasyu ON yyksyu.SyaSyuCdSeq = syasyu.SyaSyuCdSeq AND yyksho.TenantCdSeq = syasyu.TenantCdSeq
...
LEFT JOIN VPM_YoyKbn yoykbn ON yyksho.YoyaKbnSeq = yoykbn.YoyaKbnSeq AND yyksho.TenantCdSeq = yoykbn.TenantCdSeq
