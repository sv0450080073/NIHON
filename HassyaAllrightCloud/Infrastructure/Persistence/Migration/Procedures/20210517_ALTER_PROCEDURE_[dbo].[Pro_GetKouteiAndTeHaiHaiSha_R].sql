...
/****** Object:  StoredProcedure [dbo].[Pro_GetKouteiAndTeHaiHaiSha_R]    Script Date: 2021/05/17 8:47:25 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER  
PROCEDURE [dbo].[Pro_GetKouteiAndTeHaiHaiSha_R]
    @Ukeno nchar(15),
    @SyuKoYmd  char(10),
    @KikYmd    char(10),
    @TeiDanNo  smallint,
...
>0 begin
	...
	DECLARE @tblsubreport TABLE(dateKotei date,Koutei nvarchar(102),TehNm nvarchar(102),TehTel nvarchar(30), DateShow NVARCHAR(20),[ID] [int] IDENTITY(1,1) NOT NULL) WHILE (@i <= @totalRecords)
	begin
	SELECT
           @date=datehaisha
    FROM
    ...
    where
		   ...
		   SET @strSQL='' SET @strSQL = @strSQL + 'SELECT ISNULL(Kotei.datehaisha,Tehai.datehaisha) ,' + '		ISNULL(Kotei.Koutei,''''),' + '		ISNULL(Tehai.TehNm,''''),' + '		ISNULL(Tehai.TehTel,''''), '+'FORMAT (ISNULL(Kotei.datehaisha,Tehai.datehaisha), ''MM/dd'')' + ' FROM #tblKotei AS Kotei '+ @Condition + ' JOIN #tblTehaitmp AS Tehai
...
select @Rowcount=count(*) from @tblsubreport;
DECLARE @Counter INT 
SET @Counter=1
WHILE ( @Counter <= @Rowcount)
BEGIN
    update @tblsubreport set DateShow=case when ID=(select MIN(ID) from  @tblsubreport where dateKotei=(select dateKotei from @tblsubreport where ID=@Counter)) then DateShow else ''end where ID=@Counter
