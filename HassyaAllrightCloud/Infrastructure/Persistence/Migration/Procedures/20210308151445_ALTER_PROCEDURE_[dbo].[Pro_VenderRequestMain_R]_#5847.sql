...
/****** Object:  StoredProcedure [dbo].[Pro_VenderRequestMain_R]    Script Date: 3/8/2021 3:14:45 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER   PROCEDURE [dbo].[Pro_VenderRequestMain_R]
		@minUkeCd varchar(10),
		@maxUkeCd varchar(10),
		@startDate varchar(8),
		@endDate varchar(8),
...
SELECT 
	  ...
	  ,UNKOBI.HaiSJyus1        AS  'HaiSJyus1'																									
	  ,UNKOBI.HaiSJyus2        AS  'HaiSJyus2'																									
...
WHERE  
    ...
    AND EIGYOS.EigyoCd = CASE WHEN @branchId = 0 THEN EIGYOS.EigyoCd ELSE @branchId END                         --画面の受付営業所																							
    AND ((@customerStart = 0 and @customerEnd = 0)
			or
			(@customerStart <> @customerEnd and YOU_TOKISK.TokuiCd = @customerStart and YOU_TOKIST.SitenCd >= @customerSirStart)
			or
			(@customerStart <> @customerEnd and YOU_TOKISK.TokuiCd = @customerEnd and YOU_TOKIST.SitenCd <= @customerSirEnd)
			or
			(@customerStart = @customerEnd and YOU_TOKISK.TokuiCd = @customerStart and YOU_TOKIST.SitenCd >= @customerSirStart and YOU_TOKIST.SitenCd <= @customerSirEnd)
			or
			(@customerStart = 0 and @customerEnd <> 0 and ((YOU_TOKISK.TokuiCd = @customerEnd and YOU_TOKIST.SitenCd <= @customerSirEnd) or YOU_TOKISK.TokuiCd < @customerEnd))
			or
			(@customerEnd = 0 and @customerStart <> 0 and ((YOU_TOKISK.TokuiCd = @customerStart and YOU_TOKIST.SitenCd >= @customerSirStart) or YOU_TOKISK.TokuiCd > @customerStart))
			or
			(YOU_TOKISK.TokuiCd < @customerEnd and YOU_TOKISK.TokuiCd > @customerStart))
