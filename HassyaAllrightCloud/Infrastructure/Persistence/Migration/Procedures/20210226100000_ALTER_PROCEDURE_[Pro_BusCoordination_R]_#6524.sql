...
/****** Object:  StoredProcedure [dbo].[Pro_BusCoordination_R]    Script Date: 2/26/2021 9:52:00 AM ******/
SET ANSI_NULLS ON
GO
...
AS
	BEGIN TRY 	
      ...
      /* SET @ReturnCd   =   0       -- リターンコード初期化
		...
		SELECT @BookingTypeCondition = ' AND YOYKBN.YoyaKbn IN (select * from FN_SplitString('+dbo.FP_SetString(@BookingTypeList)+', ''-'')) '								
		SELECT @CustomerCondition = ' AND ((' + @CustomerFrom + ' = 0 and ' + @CustomerTo + ' = 0)
									or
									(' + @CustomerFrom + ' <> ' + @CustomerTo + ' and TOKISK.TokuiCd = ' + @CustomerFrom + ' and TOKIST.SitenCd >= ' + @SupplierFrom + ')
									or
									(' + @CustomerFrom + ' <> ' + @CustomerTo + ' and TOKISK.TokuiCd = ' + @CustomerTo + ' and TOKIST.SitenCd <= ' + @SupplierTo + ')
									or
									(' + @CustomerFrom + ' = ' + @CustomerTo + ' and TOKISK.TokuiCd = ' + @CustomerFrom + ' and TOKIST.SitenCd >= ' + @SupplierFrom + ' and TOKIST.SitenCd <= ' + @SupplierTo + ')
									or
									(' + @CustomerFrom + ' = 0 and ' + @CustomerTo + ' <> 0 and ((TOKISK.TokuiCd = ' + @CustomerTo + ' and TOKIST.SitenCd <= ' + @SupplierTo + ') or TOKISK.TokuiCd < ' + @CustomerTo + '))
									or
									(' + @CustomerTo+ ' = 0 and ' + @CustomerFrom + ' <> 0 and ((TOKISK.TokuiCd = ' + @CustomerFrom + ' and TOKIST.SitenCd >= ' + @SupplierFrom + ') or TOKISK.TokuiCd > ' + @CustomerFrom + '))
									or
									(TOKISK.TokuiCd < ' + @CustomerTo + ' and TOKISK.TokuiCd > ' + @CustomerFrom + ')) '
		SELECT @Customer01Condition = 'AND ((' + @CustomerFrom01 + ' = 0 and ' + @CustomerTo01 +  ' = 0)
									or
									(' + @CustomerFrom01 + ' <> ' + @CustomerTo01 +  ' and SIRSK.TokuiCd = ' + @CustomerFrom01 + ' and SIRST.SitenCd >= ' + @SupplierFrom01 +  ')
									or
									(' + @CustomerFrom01 + ' <> ' + @CustomerTo01 +  ' and SIRSK.TokuiCd = ' + @CustomerTo01 +  ' and SIRST.SitenCd <= ' + @SupplierTo01 +  ')
									or
									(' + @CustomerFrom01 + ' = ' + @CustomerTo01 +  ' and SIRSK.TokuiCd = ' + @CustomerFrom01 + ' and SIRST.SitenCd >= ' + @SupplierFrom01 +  ' and SIRST.SitenCd <= ' + @SupplierTo01 +  ')
									or
									(' + @CustomerFrom01 + ' = 0 and ' + @CustomerTo01 +  ' <> 0 and ((SIRSK.TokuiCd = ' + @CustomerTo01 +  ' and SIRST.SitenCd <= ' + @SupplierTo01 +  ') or SIRSK.TokuiCd < ' + @CustomerTo01 +  '))
									or
									(' + @CustomerTo01 +  ' = 0 and ' + @CustomerFrom01 + ' <> 0 and ((SIRSK.TokuiCd = ' + @CustomerFrom01 + ' and SIRST.SitenCd >= ' + @SupplierFrom01 +  ') or SIRSK.TokuiCd > ' + @CustomerFrom01 + '))
									or
									(SIRSK.TokuiCd < ' + @CustomerTo01 +  ' and SIRSK.TokuiCd > ' + @CustomerFrom01 + ')) '
		SELECT @SaleBranchCondition = CASE WHEN @SaleBranch='0' THEN ' '
											ELSE ' AND YYKSHO.UkeEigCdSeq='+@SaleBranch+' '
									END

		SELECT @UkenoCondition =  CASE WHEN @BookingFrom='0' OR @BookingTo='0'  THEN ' '
											ELSE ' AND YYKSHO.UkeCd >='+@BookingFrom+' 
    ...
    LEFT JOIN VPM_YoyKbn AS YOYKBN ON YOYKBN.YoyaKbnSeq =  YYKSHO.YoyaKbnSeq AND YOYKBN.SiyoKbn = 1 
