...
{
    public class SimpleQuotationValidator : AbstractValidator<SimpleQuotationData>
    {
        public SimpleQuotationValidator()
        {
            // Datetime
            ...
            () =>
            {
                ...
                .WithMessage(Constants.ErrorMessage.SQ_DateEndGreaterThanStart);
            });
            ...
            () =>
            {
                RuleFor(_ => _.StartPickupDate).Must((model, date) => model.StartPickupDate <= model.EndPickupDate)
                .WithMessage(Constants.ErrorMessage.SQ_DateEndGreaterThanStart);
            });
            ...
            () =>
            {
                RuleFor(_ => _.YoyakuFrom).Null().WithMessage(Constants.ErrorMessage.SQ_BookingTypeEndGreaterThanStart);
                RuleFor(_ => _.YoyakuTo).Null().WithMessage(Constants.ErrorMessage.SQ_BookingTypeEndGreaterThanStart);
            });
            ...
            () =>
            {
                ...
                .WithMessage(Constants.ErrorMessage.SQ_UkeCdEndGreaterThanStart);
            });
            RuleFor(_ => _.UkeCdFrom).MaximumLength(10);//.WithMessage(Constants.ErrorMessage.SQ_UkeCdEndGreaterThanStart);
            RuleFor(_ => _.UkeCdTo).MaximumLength(10);//.WithMessage(Constants.ErrorMessage.SQ_UkeCdEndGreaterThanStart);
            ...
            //{
            //    RuleFor(_ => _.CustomerStart).Null().WithMessage(Constants.ErrorMessage.SQ_CustomerEndGreaterThanStart);
            //    RuleFor(_ => _.CustomerEnd).Null().WithMessage(Constants.ErrorMessage.SQ_CustomerEndGreaterThanStart);
            //});
            // 仕入先
            ...
            {
                RuleFor(m => m.GyosyaShiireSakiFrom).Empty().WithMessage(Constants.ErrorMessage.SQ_CustomerEndGreaterThanStart);
                RuleFor(m => m.GyosyaShiireSakiTo).Empty().WithMessage(Constants.ErrorMessage.SQ_CustomerEndGreaterThanStart);
            });
            When(x => (x.GyosyaShiireSakiFrom != null && x.GyosyaShiireSakiTo != null && x.TokiskShiireSakiFrom != null && x.TokiskShiireSakiTo != null && (x.GyosyaShiireSakiFrom.GyosyaCd == x.GyosyaShiireSakiTo.GyosyaCd && x.TokiskShiireSakiFrom.TokuiCd > x.TokiskShiireSakiTo.TokuiCd))
                , () =>
                {
                    RuleFor(m => m.TokiskShiireSakiFrom).Empty().WithMessage(Constants.ErrorMessage.SQ_CustomerEndGreaterThanStart);
                    RuleFor(m => m.TokiskShiireSakiTo).Empty().WithMessage(Constants.ErrorMessage.SQ_CustomerEndGreaterThanStart);
                });
            When(x => ((x.GyosyaShiireSakiFrom != null && x.GyosyaShiireSakiTo != null && x.TokiskShiireSakiFrom != null && x.TokiskShiireSakiTo != null && x.TokiStShiireSakiFrom != null && x.TokiStShiireSakiTo != null)
                    ...
                    {
                        RuleFor(m => m.TokiStShiireSakiFrom).Empty().WithMessage(Constants.ErrorMessage.SQ_CustomerEndGreaterThanStart);
                        RuleFor(m => m.TokiStShiireSakiTo).Empty().WithMessage(Constants.ErrorMessage.SQ_CustomerEndGreaterThanStart);
                    });
            ...
            {
                RuleFor(_ => _.BranchStart).Null().WithMessage(Constants.ErrorMessage.SQ_BranchEndGreaterThanStart);
                RuleFor(_ => _.BranchEnd).Null().WithMessage(Constants.ErrorMessage.SQ_BranchEndGreaterThanStart);
            });
        }
