...
{
    public class DepositListValidator : AbstractValidator<DepositListSearchModel>
    {
        public DepositListValidator()
        {
            ...
            When(x => x.StartReservationClassification != null && x.EndReservationClassification != null && x.StartReservationClassification.YoyaKbn > x.EndReservationClassification.YoyaKbn, () =>
            {
                RuleFor(x => x.StartReservationClassification).Empty().WithMessage("BI_T005");
                RuleFor(x => x.EndReservationClassification).Empty().WithMessage("BI_T005");
            });
            When(x => x.StartTransferBank != null && x.EndTransferBank != null && Int32.Parse(x.StartTransferBank.Code) > Int32.Parse(x.EndTransferBank.Code), () => 
            {
                RuleFor(x => x.EndTransferBank).Empty().WithMessage("BI_T006");
                RuleFor(x => x.StartTransferBank).Empty().WithMessage("BI_T006");
            });
        }
    }
}
