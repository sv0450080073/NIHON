...
{
    public class GetDataForCmbBillAddressDistinctQuery : IRequest<List<BillAddress>>
    {
        public BillsCheckListFormData billCheckListData { get; set; }
        public int companyId { get; set; }
        public int tenantId { get; set; }
        private readonly KobodbContext _dbContext;
        public class Handler : IRequestHandler<GetDataForCmbBillAddressDistinctQuery, List<BillAddress>>
        {
            private readonly KobodbContext _dbContext;
            ...
            public async Task<List<BillAddress>> Handle(GetDataForCmbBillAddressDistinctQuery request, CancellationToken cancellationToken)
            {
                List<BillAddress> list = new List<BillAddress>();
                using (var command = _dbContext.Database.GetDbConnection().CreateCommand())
                {
                    CreateParameter(command, "PK_dBillCheckListForDistinctBillAddress_R", request.billCheckListData, request.companyId, request.tenantId);
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            if((short)reader["TokuiCd"] != 0)
                            {
                                list.Add(new BillAddress
                                {
