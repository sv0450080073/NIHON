...
{
    public class GetBillingListCsvAsyncQuery : IRequest<List<BillingListGridCsvData>>
    {
        public BillingListFilter billingListFilter { get; set; }
        public class Handler : IRequestHandler<GetBillingListCsvAsyncQuery, List<BillingListGridCsvData>>
        {
            ...
            public async Task<List<BillingListGridCsvData>> Handle(GetBillingListCsvAsyncQuery request, CancellationToken cancellationToken = default)
            {
                if(request.billingListFilter == null)
                    return new List<BillingListGridCsvData>();
                ...
                List<BillingListGridCsvData> billingListGridCsvDatas = new List<BillingListGridCsvData>();
                ...
                await _context.LoadStoredProc(request.billingListFilter.TransferAmountOutputClassification?.Code == 1 ?
                    "PK_dBillingListGridWithoutOutputClassificationCsv_R" : "PK_dBillingListGridWithOutputClassificationCsv_R")
                                ...
                                .ExecAsync(async r => billingListGridCsvDatas = await r.ToListAsync<BillingListGridCsvData>());
                return billingListGridCsvDatas;
            }
        }
    }
}
