...
{
    public class GetSuperMenuReservationDataReportQuery : IRequest<List<SuperMenuReservationReportPDF>>
    {
        ...
        public class Handler : IRequestHandler<GetSuperMenuReservationDataReportQuery, List<SuperMenuReservationReportPDF>>
        {
            ...
            public async Task<List<SuperMenuReservationReportPDF>> Handle(GetSuperMenuReservationDataReportQuery request, CancellationToken cancellationToken)
            {
                List<SuperMenuReservationData> list = new List<SuperMenuReservationData>();
                List<SuperMenuReservationReportPDF> listReport = new List<SuperMenuReservationReportPDF>();
                var currentDate = DateTime.Now.ToString(CommonConstants.FormatYMDHm);
                var page = 1;
                using (var command = _dbContext.Database.GetDbConnection().CreateCommand())
                {
                    CreateParameter(command, "PK_SpMnYykReport_R", request.hyperData, request.companyId, request.tenantId);
                    command.CommandText += " "
                        + "  WITH RECOMPILE";
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            list.Add(new SuperMenuReservationData
                            {
                                ...
                                ReceptionDate = DateDisplayValue((string)reader["UkeYmd"]).Value.ToString("yyyyMMdd"),
                                ...
                                DepartureFlag = (int)reader["Cnt_SaikFlg"],
                                BusNumberFlag = (int)reader["Cnt_DaiSuFlg"],
                                AmountFlag = (int)reader["Cnt_KingFlg"],
                                ScheduleFlag = (int)reader["Cnt_NitteiFlg"],
                                ConfirmationCount = (byte)reader["KaknKais"],
                                ConfirmFlag = (string)reader["HaiSYmd"],
                                Supplier = (string)reader["SirRyakuNm"] + " " + (string)reader["SirSitenRyakuNm"],
                ...
                using (var command = _dbContext.Database.GetDbConnection().CreateCommand())
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            list[GridIndex].SymbolInString += (string)reader["SyaSyuNm"] + "\r\n";
                            list[GridIndex].Y0 += ((short)reader["SyaSyuDai"]).ToString() + "台" + "\r\n";
                ...
                var onePage = new SuperMenuReservationReportPDF();
                if (list.Count >= 16)
                {
                    var count = Math.Ceiling(list.Count / 16 * 1.0);
                    for (int i = 0; i < count; i++)
                    {
                        onePage = new SuperMenuReservationReportPDF();
                    ...
                    if (count * 16 < list.Count)
                    {
                        ...
                        {
                            listPerPage.Add(new SuperMenuReservationData());
                        }
                        SetData(onePage, list, listPerPage, currentDate, request.hyperData, page);
                        listReport.Add(onePage);
                        page++;
            ...
            private void SetData(SuperMenuReservationReportPDF onePage, List<SuperMenuReservationData> list,
                List<SuperMenuReservationData> listPerPage, string currentDate, HyperFormData listSearch, int page)
            {
                int i = 1;
                list.ForEach(e => {
