...
{
    public class GetSuperMenuReservationTotalGridData : IRequest<SuperMenuReservationTotalGridData>
    {
        public HyperFormData hyperData { get; set; }
        public int companyId { get; set; }
        public int tenantId { get; set; }
        public class Handler : IRequestHandler<GetSuperMenuReservationTotalGridData, SuperMenuReservationTotalGridData>
        {
            ...
            public async Task<SuperMenuReservationTotalGridData> Handle(GetSuperMenuReservationTotalGridData request, CancellationToken cancellationToken)
            {
                SuperMenuReservationTotalGridData totalGridData = new SuperMenuReservationTotalGridData();
                using (var command = _dbContext.Database.GetDbConnection().CreateCommand())
                {
                    CreateParameter(command, "PK_SpMnYykSum_R", request.hyperData, request.companyId, request.tenantId);
                    _dbContext.Database.OpenConnection();
                    var totalRow = command.ExecuteReader();
                    if (totalRow.Read())
                    {
                        totalGridData = new SuperMenuReservationTotalGridData()
                        {
                            Order = new SuperMenuReservationTotalData
                            {
                                Fare = (long)totalRow["SyaRyoUnc"],
                                FareTax = (long)totalRow["SyaRyoSyo"],
                                TaxIncluded = (long)totalRow["SyaRyoUnc"] + (long)totalRow["SyaRyoSyo"],
                                FareFee = (long)totalRow["SyaRyoTes"],
                                Guide = (long)totalRow["Gui_UriGakKin_S"],
                                GuideTax = (long)totalRow["Gui_SyaRyoSyo_S"],
                                GuideFee = (long)totalRow["Gui_SyaRyoTes_S"],
                                Other = (long)totalRow["UriGakKin_S"],
                                OtherTax = (long)totalRow["SyaRyoSyo_S"],
                                OtherFee = (long)totalRow["SyaRyoTes_S"]
                            },
                            Company = new SuperMenuReservationTotalData
                            {
                                Fare = (long)totalRow["JiSyaRyoUnc"],
                                FareTax = (long)totalRow["JiSyaRyoSyo"],
                                FareFee = (long)totalRow["JiSyaRyoTes"]
                            },
                            Bus = new SuperMenuReservationTotalData
                            {
                                Fare = (long)totalRow["YoushaUnc"],
                                FareTax = (long)totalRow["YoushaSyo"],
                                FareFee = (long)totalRow["YoushaTes"],
                                Guide = (long)totalRow["YGui_HaseiKin_S"],
                                GuideTax = (long)totalRow["YGui_SyaRyoSyo_S"],
                                GuideFee = (long)totalRow["YGui_SyaRyoTes_S"],
                                Other = (long)totalRow["FGui_HaseiKin_S"],
                                OtherTax = (long)totalRow["FGui_SyaRyoSyo_S"],
                                OtherFee = (long)totalRow["FGui_SyaRyoTes_S"]
                            }
                        };
                    }
                    totalRow.Close();
                    _dbContext.Database.CloseConnection();
                    return await Task.FromResult(totalGridData);
                }
            }
        }
    }
