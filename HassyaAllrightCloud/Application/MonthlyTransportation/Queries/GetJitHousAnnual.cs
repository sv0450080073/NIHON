...
{
    public class GetJitHousAnnual : IRequest<List<JitHouReports>>
    {
        public SearchParam SearchParam { get; set; }
        public class Handler : IRequestHandler<GetJitHousAnnual, List<JitHouReports>>
        {
            ...
            public async Task<List<JitHouReports>> Handle(GetJitHousAnnual request, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    await _context.LoadStoredProc("dbo.PK_SpJitHouAnnual_R")
                       .AddParam("@StrDate", request?.SearchParam?.StrDate ?? "")
                       .AddParam("@EndDate", request?.SearchParam?.EndDate ?? "")
                        ...
                        foreach (var groupJitHouResult in jitHouResult)
                        {
                            ...
                            foreach (var group in groupReport)
                            {
                                jitHouReportItems.Add(new JitHouReportItem
                                {
                                    ...
                                    ProcessingDate = request?.SearchParam?.StrDate,
                                    ProcessingDateEnd = request?.SearchParam?.EndDate,
                                    ...
                                    Medium_UnkoJisyaKm = (group.FirstOrDefault(x => x.KataKbnNm == "中型")?.UnkoCnt > 0   && group.FirstOrDefault(x => x.KataKbnNm == "中型") != null) ? ((decimal)(group.FirstOrDefault(x => x.KataKbnNm == "中型")?.JitJisaKm) / (decimal)(group.FirstOrDefault(x => x.KataKbnNm == "中型")?.UnkoCnt)).ToString("#,##0.0") : "0.0",
