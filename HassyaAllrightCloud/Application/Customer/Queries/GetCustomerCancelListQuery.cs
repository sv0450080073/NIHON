...
{
    public class GetCustomerCancelListQuery : IRequest<List<LoadCustomerList>>
    {
        private readonly int _tenantId;
        private readonly string _fromDate;
        private readonly string _toDate;

        public GetCustomerCancelListQuery(int tenantId, string fromDate, string toDate)
        {
            _tenantId = tenantId;
            _fromDate = fromDate;
            _toDate = toDate;
        ...
        public class Handler : IRequestHandler<GetCustomerCancelListQuery, List<LoadCustomerList>>
        {
            private readonly KobodbContext _context;
            private readonly ILogger<GetCustomerCancelListQuery> _logger;

            public Handler(KobodbContext context, ILogger<GetCustomerCancelListQuery> logger)
            {
                _context = context;
                _logger = logger;
            }

            public async Task<List<LoadCustomerList>> Handle(GetCustomerCancelListQuery request, CancellationToken cancellationToken)
            {
                try
                {
