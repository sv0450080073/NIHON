...
{
    public class GetAppointmentListsCustomGroup : IRequest<List<AppointmentList>>
    {
        ...
        public class Handler : IRequestHandler<GetAppointmentListsCustomGroup, List<AppointmentList>>
        {
            ...
            public async Task<List<AppointmentList>> Handle(GetAppointmentListsCustomGroup request, CancellationToken cancellationToken)
            {
                List<AppointmentList> result = new List<AppointmentList>();

                List<AppointmentList> yoteiData = GetYoteiCustomGroupData(request.FromDate, request.ToDate, request.EmployeeId, request.GroupId, request.PlanTypes, request.AppointmentLabels);
                List<AppointmentList> finalYoteiData = GetSchYotKSyaCustomGroupFbData(request.FromDate, request.ToDate, request.GroupId, request.EmployeeId, yoteiData);
                List<AppointmentList> kinkyuData = GetKinkyujCustomGroupData(request.GroupId, request.EmployeeId, request.FromDate, request.ToDate);
                List<AppointmentList> haiinData = GetHaiinCustomGroupData(request.TenantCdSeq, request.EmployeeId, request.GroupId, request.FromDate, request.ToDate);
            ...
            public List<AppointmentList> GetYoteiCustomGroupData(string fromDate, string toDate, int employeeId, int groupId, List<PlanType> planTypes, List<AppointmentLabel> appointmentLabels)
            {
                List<AppointmentList> result = new List<AppointmentList>();

                List<YoteiDataModel> dataModels = new List<YoteiDataModel>();

                _dbcontext.LoadStoredProc("PK_dSchYoteiCustomGroup_R")
                ...
                foreach (var item in dataModels)
                {
                    ...
                    if (item.GaiKkKbn != 1 && !item.YotKSya.Contains(new HassyaAllrightCloud.Domain.Dto.ClaimModel().SyainCdSeq.ToString()))
                    {
                        ...
                        result.Add(new AppointmentList()
                        {
                            ...
                            Staffs = item.YotKSya.Split(',').Select(Int32.Parse).ToList(),
                            DataType = 1,
                            DisplayType = 90,
            ...
            public List<AppointmentList> GetSchYotKSyaCustomGroupFbData(string fromDate, string toDate, int groupId, int employeeId, List<AppointmentList> yoteis)
            {
                List<SchYotKSyaFbDataModel> dataModels = new List<SchYotKSyaFbDataModel>();

                _dbcontext.LoadStoredProc("PK_dSchYoteiCustomGroupFb_R")
                          .AddParam("GroupId", groupId)
                          .AddParam("EmployeeId", employeeId)
                          .AddParam("FromDate", fromDate)
                          .AddParam("ToDate", toDate)
            ...
            public List<AppointmentList> GetKinkyujCustomGroupData(int groupId, int employeeId, string fromDate, string toDate)
            {
                List<AppointmentList> result = new List<AppointmentList>();

                List<KinkyuDataModel> dataModels = new List<KinkyuDataModel>();

                _dbcontext.LoadStoredProc("PK_dGetKikyujCustomGroup_R")
                          .AddParam("EmployeeId", employeeId)
                          .AddParam("FromDate", fromDate)
                          .AddParam("ToDate", toDate)
                          .AddParam("GroupId", groupId)
                ...
                foreach (var item in dataModels)
                {
                    ...
                    result.Add(new AppointmentList()
                    {
                        ...
                        StartDateDisplay = DateTime.ParseExact(item.KinKyuSYmd + $"{Int32.Parse(string.IsNullOrWhiteSpace(item.KinKyuSTime) ? "0" : item.KinKyuSTime):0000}", "yyyyMMddHHmm", null),
                        EndDateDisplay = DateTime.ParseExact(item.KinKyuEYmd + $"{Int32.Parse(string.IsNullOrWhiteSpace(item.KinKyuETime) ? "0" : item.KinKyuETime):0000}", "yyyyMMddHHmm", null),
            ...
            public List<AppointmentList> GetHaiinCustomGroupData(int tenantCdSeq, int employeeId, int groupId, string fromDate, string toDate)
            {
                List<AppointmentList> result = new List<AppointmentList>();

                List<HaiinDataModel> dataModels = new List<HaiinDataModel>();

                _dbcontext.LoadStoredProc("PK_dGetHaiinCustomGroup_R")
                         .AddParam("EmployeeId", employeeId)
                          .AddParam("FromDate", fromDate)
                          .AddParam("ToDate", toDate)
                ...
                foreach (var item in dataModels)
                {
                    ...
                    result.Add(new AppointmentList()
                    {
                        ...
                        StartDateDisplay = DateTime.ParseExact(item.SyuKoYmd + $"{Int32.Parse(string.IsNullOrWhiteSpace(item.SyuKoTime) ? "0" : item.SyuKoTime):0000}", "yyyyMMddHHmm", null),
                        EndDateDisplay = DateTime.ParseExact(item.KikYmd + $"{Int32.Parse(string.IsNullOrWhiteSpace(item.KikTime) ? "0" : item.KikTime):0000}", "yyyyMMddHHmm", null),
                        Typelabel = new LabelList()
                        {
                            LabelType = 6,
