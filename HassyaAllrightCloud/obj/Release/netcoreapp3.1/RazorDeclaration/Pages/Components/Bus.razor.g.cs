#pragma checksum "E:\Project\HassyaAllrightCloud\Pages\Components\Bus.razor" "{ff1816ec-aa5e-4d10-87f7-6f4963833460}" "b3cf38efff8d517af7873daf1a55574ff452e569"
// <auto-generated/>
...
#line 546 "E:\Project\HassyaAllrightCloud\Pages\Components\Bus.razor"
    ...
    [Parameter] public TPM_CodeKbData CodeKbnItem { get; set; }
    ...
    public string GetheaderText(DateTime date)
    {
        ...
        int countbusnameLarge = 110;
        int countbusnameMedium = 110;
        int countbusnameSmall = 110;
        if(countbusname!=0)
        {
            int a = countbusname;
        }
        int countbus = 0;
        int countbusLarge = 100;
        int countbusMedium = 100;
        int countbusSmall = 100;
        ...
        {
            var busitem = BusLines.Where(t => t.KSKbn == 2 && String.Compare(t.StartDateDefault, datestr) <= 0 && String.Compare(t.EndDateDefault, datestr) >= 0 && listbranch.Contains(t.BranchId)).ToList();
            countbus =busitem.Select(t=>t.BusLine).Distinct().Count();
            if (busitem.Count != 0)
            {
            ...
            if (listKyoSHebyDate != null)
            {
                ...
                if (listKyoSHebyDate.Count != 0 && listKyoSHebyDate.Where(t => t.date == date).Count() > 0)
                {
                    ...
                    countBusdriver = listKyoSHebyDateitem.Where(t => t.SyokumuCdSeq == 1 || t.SyokumuCdSeq == 2).Count();
                    countBusguider = listKyoSHebyDateitem.Where(t => t.SyokumuCdSeq == 3 || t.SyokumuCdSeq == 4).Count();
                }
        ...
        {
            var busitem = BusLines.Where(t => t.KSKbn == 2 && String.Compare(t.StartDateDefault, datestr) <= 0 && String.Compare(t.EndDateDefault, datestr) >= 0 && t.BranchId==GroupID && listbranch.Contains(t.BranchId) ).ToList();
            countbus =busitem.Select(t=>t.BusLine).Distinct().Count();
            if (busitem.Count != 0)
            {
                foreach(var item in busitem)
            ...
            if (listKyoSHebyDate != null)
            {
                ...
                if (listKyoSHebyDate.Count != 0 && listKyoSHebyDate.Where(t => t.date == date).Count() > 0)
                {
                    var a = listKyoSHebyDate.Where(t => t.date == date).ToList();
                    var listKyoSHebyDateitem = listKyoSHebyDate.Where(t => t.date == date).First().KyoSHelst;
                    busdriver = listKyoSHebyDateitem.Where(t => (t.SyokumuCdSeq == 1 || t.SyokumuCdSeq == 2) && t.EigyoCdSeq == GroupID).ToList();
                    busguider = listKyoSHebyDateitem.Where(t => (t.SyokumuCdSeq == 3 || t.SyokumuCdSeq == 4) && t.EigyoCdSeq == GroupID).ToList();
        ...
        {
            var busitem = BusLines.Where(t => t.KSKbn == 2 && String.Compare(t.StartDateDefault, datestr) <= 0 && String.Compare(t.EndDateDefault, datestr) >= 0 && t.CompanyId==GroupID && listbranch.Contains(t.BranchId)).ToList();
            countbus =busitem.Select(t=>t.BusLine).Distinct().Count();
            if (busitem.Count != 0)
            {
                foreach(var item in busitem)
            ...
            if (listKyoSHebyDate != null)
            {
                ...
                if (listKyoSHebyDate.Count != 0 && listKyoSHebyDate.Where(t => t.date == date).Count() > 0)
                {
                    var a = listKyoSHebyDate.Where(t => t.date == date).ToList();
                    var listKyoSHebyDateitem = listKyoSHebyDate.Where(t => t.date == date).First().KyoSHelst;
                    busdriver = listKyoSHebyDateitem.Where(t => (t.SyokumuCdSeq == 1 || t.SyokumuCdSeq == 2) && t.CompanyCdSeq == GroupID).ToList();
                    busguider = listKyoSHebyDateitem.Where(t => (t.SyokumuCdSeq == 3 || t.SyokumuCdSeq == 4) && t.CompanyCdSeq == GroupID).ToList();
                    busdriverlst = busdriver.Select(t => t.SyainCdSeq).ToList();
                    busguiderlst=busguider.Select(t => t.SyainCdSeq).ToList();
                    countBusdriver = busdriver.Count();
                    countBusguider = busguider.Count();
        ...
        tooltip += "<div>" + (countbusnameLarge - countbusLarge) + "台　大型（" + countbusLarge + "/" + countbusnameLarge + "）</div>";
        tooltip += "<div>" + (countbusnameMedium - countbusMedium) + "台　中型（" + countbusMedium + "/" + countbusnameMedium + "）</div>";
        tooltip += "<div>" + (countbusnameSmall - countbusSmall) + "台　小型（" + countbusSmall + "/" + countbusnameSmall + "）</div>";
        tooltip += "</div></div>";
        return "<a href='javascript:void(0)' data-toggle='tooltip' data-html='true' data-placement='right' data-original-title='"+tooltip+"'>空台数：" + (countbusname - countbus).ToString() + "台（" + countbus + "/" + countbusname + "）</a><div>空運転手：" + (countBusdriver - countbusdriveruse - countBusdriveroff).ToString() + "人（" + (countbusdriveruse + countBusdriveroff).ToString() + "/" + countBusdriver + "）</div><div>空ガイド：" + (countBusguider - countbusguideruse - countBusguieroff).ToString() + "人（" + (countbusguideruse + countBusguieroff).ToString() + "/" + countBusguider + "）</div>";
    }
    ...
    public async Task HandlerStatusUpdated(ItemBus updatedBus)
    {
        try
        {
            ...
            {
                TKD_HaishaDataService.Updatebusdatagray(updatedBus, 1);
    ...
    protected override void OnParametersSet()
    {
        ...
        {
            BusLines = BusLines.Where(t => t.BookingType == Param.ActiveR).ToList();
        }
