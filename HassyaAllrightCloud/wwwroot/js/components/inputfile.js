(function () {
    window.BlazorInputFile = {
        ...
        init: function init(elem, componentInstance) {
            ...
            elem.onchange = function handleInputFileChange(event) {
                // Reduce to purely serializable data, plus build an index by ID
                elem._blazorFilesById = {};
                fileList = Array.prototype.map.call(elem.files, function (file) {
                    var result = {
                ...
                for (let item of list) {
                    ...
                    componentInstance.invokeMethodAsync('NotifyChange', item, isEnd).then(function () {
            ...
            if (!isFirefox) {
                elem.onclick = function () {
                    elem.onfocus = function () {
                        if (!BlazorInputFile.firstChange) {
                            BlazorInputFile.firstChange = true;
                            componentInstance.invokeMethodAsync('SelectedEmptyFile');
                        }
                    }
                };
            }
    ...
    function getArrayBufferFromFileAsync(elem, fileId) {
        ...
        if (!file.readPromise) {
            file.readPromise = new Promise(function (resolve, reject) {
                ...
                reader.onerror = function (err) { console.log(err); reject(err); };
